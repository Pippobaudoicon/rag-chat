...
3b. Once gated, the mutually independent preamble reads run concurrently in a single `Promise.all` rather than serially: conversation + messages load (ownership 404 enforced after), language routing, the memory brief, and user preferences. This collapses `preStreamMs` from the sum of those phases toward their max. Routing runs in the same batch for the common case; for regenerate requests whose body omits the question text (resolved from history), the single routing call is deferred until the question is known.
