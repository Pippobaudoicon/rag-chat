...
- **Guests see that Search, Memory and Billing need an account.** For signed-out visitors, the three sidebar buttons show a small lock. Tapping one no longer loads the page only to be sent to sign-in, which briefly showed the page's loading skeleton. Instead, a card opens with a one-line reason and a "Sign up free" button that goes to `/sign-up`. The card is a Base UI `Popover` (`FeatureGate` in `src/components/ui/feature-gate.tsx`; generic, so a Pro-only button can reuse it with `href="/billing"`), with copy in `UI_TEXT[lang].sidebar.guestLocked`. The chat's other links for guests already pointed at `/sign-up`.
- **Auth checks moved out of the proxy.** Clerk 7.9 deprecates `createRouteMatcher` (it logged a warning on every start) and recommends checking auth where the data is read. `src/proxy.ts` now does no auth checks. It still claims guest chats after sign-in, and it hands out the guest cookie on the guest routes, now using a plain path check (`GUEST_PATHS`). The API routes already checked auth themselves (`getViewer()`/`auth()`). The Search, Memory and Billing pages now call `auth.protect()`. Search had no check of its own before. Memory and Billing used to redirect to a bare `/sign-in`; now they keep the return URL. Behaviour changes for signed-out visitors: those pages redirect after streaming starts (Next inserts the redirect under the route's `loading.tsx`) instead of with an HTTP 307. `/api/memory` and `/api/search` return 401 instead of redirecting to sign-in, and unknown paths show the 404 page. `/` no longer sets the guest cookie; `next.config.ts` redirects it to `/chat`, which does.

## 0.12.51

- **The app's default language comes from the device too.** Until the user picks a language, `LanguageProvider` now uses the device language (`navigator.languages`) instead of always Italian, and so does the PWA install prompt. Detection is the same `pickLanguage()` (`src/components/chat/i18n.ts`) the auth pages use, so a device gets the same language before and after login. The auto-pick isn't saved, so it keeps following the device until the user chooses one, which is saved as before (`chat:language`). Leaving the app now restores `<html lang>` to its server-rendered value rather than a hardcoded `"it"`.
- **Sign-in and sign-up follow the device language.** There is no language picker before login, so the root layout now reads `Accept-Language` (`deviceLanguage()` in `AuthShell.tsx`). It takes the first language the app has copy for (Italian, English, Spanish), falling back to English, and uses it for all Clerk text (`@clerk/localizations`: `itIT`/`enUS`/`esES`, including labels, errors, and verification steps), our start-screen titles, the guest link, the privacy note, and D&C 88:118 (official wording per language). `<html lang>` gets the same language on SSR. The auth copy lives in `UI_TEXT[lang].auth` (`src/components/chat/i18n.ts`). `UI_LANGUAGE_BCP47` moved to `src/lib/types.ts` so server code can import it. Clerk is upgraded from `@clerk/nextjs` 7.4.1 to 7.9.5 (same major), so it shares one `@clerk/shared` (4.34) with `@clerk/localizations` 4.18.0. With mismatched `@clerk/shared` versions the localization types don't typecheck, so bump the two packages together. Side effects: the root layout reading headers makes the previously static routes (`/sign-in`, `/sign-up`, `/privacy-policy`) render per request, and the in-app Clerk `UserButton` menu also follows the device language.
