...
- **Parallelize the chat pre-stream preamble (Phase 2 · P0-B).** `POST /api/chat` previously awaited a chain of mostly-independent preamble operations serially: conversation load → messages load → language routing → memory brief → user prefs, each in its own latency phase. These reads are mutually independent, so they now run concurrently in a single `Promise.all`, collapsing `preStreamMs` from roughly the **sum** of those phases toward their **max**. Individual `phases.*` durations are unchanged; only their overlap improved. No behavior change otherwise.
  - **Gate ordering preserved.** The `auth` (401) and `ratelimit` (429) gates still resolve *before* the parallel batch, so a rejected request never speculatively pays for the language-routing LLM call or the DB reads. Tradeoff noted for review: `entitlements` + `ratelimit` stay sequential ahead of the batch (rather than overlapping the DB reads) specifically to honor "don't burn LLM cost on a request you're about to reject" — the routing LLM call dominates the batch anyway, so the DB reads hide behind it.
