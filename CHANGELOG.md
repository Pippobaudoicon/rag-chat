...
- **Eager/speculative retrieval (Phase 2 · P1, flag-gated, default ON).** For common topical questions, `POST /api/chat` now runs the default `semantic_search` retrieval **during the preamble** (on an answer-cache miss) and seeds the resulting chunks as the user message's `initialChunks`, so the model can answer on **turn 1** instead of first spending an empty tool-decision model round-trip (the `firstToolCallMs − preStreamMs` gap the latency trace measures). New `eagerRetrieval` latency phase records the cost.
  - **No double-retrieval, no cache divergence.** Eager and lazy (`semantic_search` tool) paths now share `runSemanticRetrieval()` (`src/lib/rag/tools/shared/semantic-retrieval.ts`) — the full `cacheKey → getFromCache → (retrieve + expandRelatedContext + warm setInCache) → graphRerank` pipeline, everything the tool did except `context.registerChunks`. Eager warms the **same** Upstash cacheKey the tool uses, so if the model still calls `semantic_search` it resolves as a cache hit rather than re-retrieving. Citation indices stay identical because both paths produce the same ordered chunks and `createRagToolContext` seeds `initialChunks` in the same order `formatContext` numbers them.
  - **Eligibility gate.** Eager runs only when `isEagerRetrievalEnabled()` **and** not the fixed-chunks regenerate path (`hasFixedChunks`) **and** the query is not a scripture reference (`parseScriptureSelection()` non-null → `lookup_scripture_passage` handles it) **and** at least one source is selected. Placed after the answer-cache lookup so cache-hit / regenerate turns never pay for it.
  - **Kill-switch:** `RAG_EAGER_RETRIEVAL=false` disables it (defaults on, per the closed-beta rollout decision: ship active behind a kill-switch, not dark). Deliberately **excluded** from `retrievalFlagsSignature()` — it changes *when* retrieval runs, not the cached results, so it must not fork the cache namespace.
  - **Tests/guards:** new `npm run test:eager` (`scripts/test/eager-eligibility.test.ts`) — a pure, network-free regression asserting the scripture-skip classification (refs → skip, topical → eligible) in both English and Italian. `npm run typecheck` clean.
